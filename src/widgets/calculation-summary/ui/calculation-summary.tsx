...
import { Alert, Card, Descriptions, Skeleton, Tag, Tooltip, Typography } from 'antd'
...
export function CalculationSummary({ data, loading }: CalculationSummaryProps) {
  const isZeroResult = data != null && data.totalPrice === 0
          ...
          <>
            ...
            {isZeroResult && (
              <Alert
                type="error"
                showIcon
                message="Недостаточно данных или слишком большой разброс выборки"
                style={{ marginTop: 8 }}
              />
            )}
          </>
